...
// exit and makes typos / unknown-subcommand probes look like success.
//
...
// label name lets us write "<run-id> is required" or similar. Single-line
// — error output stays one line so consumers parse stderr the same shape
// they parse for every other client-side validation.
func requireOneArg(label string) cobra.PositionalArgs {
	return func(_ *cobra.Command, args []string) error {
		...
		case 1:
			return nil
		default:
			return fmt.Errorf("expected exactly one %s, got %d", label, len(args))
		}
	}
}
