...
func buildClierPrompt(teamName string, m MemberData, nameByID map[string]string) string {
	var b strings.Builder

	// Header + intro
	b.WriteString("# Team Protocol\n\n")
	fmt.Fprintf(&b, "You are **%s**, a member of team **%s**.\n", m.MemberName, teamName)
	b.WriteString("This protocol defines your role, team structure, and how to coordinate with teammates.\n")

	// Team Structure
	b.WriteString("\n## Team Structure\n")
	writeRelNames(&b, "Leaders", m.Relations.Leaders, nameByID)
	writeRelNames(&b, "Workers", m.Relations.Workers, nameByID)
	writeRelNames(&b, "Peers", m.Relations.Peers, nameByID)
	if len(m.Relations.Leaders) == 0 && len(m.Relations.Workers) == 0 && len(m.Relations.Peers) == 0 {
		b.WriteString("(none)\n")
	}
	...
	writeSendCommands(&b, m.Relations, nameByID)
	if len(m.Relations.Leaders) == 0 {
		fmt.Fprintf(&b, "Send to user:\n```bash\nclier session send --to %s \"<message>\"\n```\n", domain.UserMemberID)
	}

	// Operating Rules
	b.WriteString("\n## Operating Rules\n")
	if len(m.Relations.Workers) > 0 {
