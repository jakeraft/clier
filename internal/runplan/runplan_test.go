...
// PurgeRun removes the entire run directory — clones, protocols, and
// run.json itself. Stop is final; a stopped run leaves no trace.
func TestPurgeRunRemovesRunDir(t *testing.T) {
	root := t.TempDir()
	store := NewStore(root)
	plan := &Plan{
		RunID:     "rid",
	...
	if err := store.PurgeRun(plan); err != nil {
		t.Fatalf("PurgeRun: %v", err)
	}

	if _, err := os.Stat(plan.RunDir); !os.IsNotExist(err) {
		t.Errorf("run dir should be gone, stat err: %v", err)
	}
	if _, err := store.Load("rid"); !errors.Is(err, ErrRunNotFound) {
		t.Errorf("expected ErrRunNotFound after purge, got %v", err)
	}
}
