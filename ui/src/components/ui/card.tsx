...
function Card({ className, ...properties }: React.ComponentProps<"div">) {
  return (
    <div
      data-slot="card"
      className={cn(
        // rounded-base: project-wide container radius (override shadcn rounded-xl)
        "rounded-base bg-card text-card-foreground flex flex-col gap-6 border py-6 shadow-sm",
        className,
      )}
      {...properties}
    />
