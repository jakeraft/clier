...
type Entity = "team" | "task" | "member" | "agent-dot-md" | "skill" | "claude-settings" | "claude-json" | "git-repo" | "env";
...
function entityFromPath(path: string): Entity | undefined {
  const match = /^\/(teams|tasks|members|prompts|claude|agent-dot-mds|skills|claude-settings|claude-jsons|git-repos|envs)(\/|$)/.exec(path);
