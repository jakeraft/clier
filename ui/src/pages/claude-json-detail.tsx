...
import { ClipboardList, FileJson } from "lucide-react";
import { api } from "@/api";
...
export function ClaudeJsonDetail() {
  const { id: parameterId } = useParams<{ id: string }>();
  const { data, error, loading } = useDetailPage(parameterId, api.claudeJsons.get);
  ...
  return (
    <DetailLayout error={error}>
      ...
      <Section icon={FileJson} title="Content">
